...
# 14,'烧菜用的作料如何收纳比较好？',27566979,6,2,1421966997,1421133097,0
...
class UpdateOneQuestion(threading.Thread):
    def __init__(self, queue):
        ...
        #                           db=db_name, charset=charset, use_unicode=use_unicode)
        self.cursor = self.db.cursor()
        myclient = pymongo.MongoClient('mongodb://localhost:3001/')
        meteorDB = myclient['meteor']

        self.qColltion = meteorDB['question']
    ...
    def update(self, link_id, count_id):
        ...
        if focus_amount == u'\\xe8\\xbf\\x98\\xe6\\xb2\\xa1\\xe6\\x9c\\x89':
            return
        ...
        if count_id % 1 == 0:
            print(str(count_id) + " , " + self.getName() + " Update QUESTION set FOCUS = " + focus_amount + " , ANSWER = " + answer_amount +
                  ", LAST_VISIT = " + str(time_now) + ", TOP_ANSWER_NUMBER = " + str(top_answer_votes) + " where LINK_ID = " + link_id)
        ...
        self.cursor.execute(sql, (focus_amount, answer_amount,
                            time_now, top_answer_votes, link_id))
...
class UpdateQuestions:
    def __init__(self):
        ...
        self.question_thread_amount = int(
            cf.get("question_thread_amount", "question_thread_amount"))
